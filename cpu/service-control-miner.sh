...
if [ "$CURRENT_IP" == "$TARGET_IP" ]; then
    ...
    nice -n -20 "$PEPOW_BINARY" --disable-gpu --algorithm "$PEPOW_ALGO" --pool "$PEPOW_POOL" --wallet "$PEPOW_WALLET.$(hostname)" --password PEPEW --cpu-threads $TOTAL_THREADS --keepalive true --randomx-use-1gb-pages --cpu-threads-priority 5 >> "$PEPOW_LOGFILE" 2>> /var/log/start-deroluna-errors.log &
