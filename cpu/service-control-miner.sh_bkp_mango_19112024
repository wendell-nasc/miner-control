...
if [ -x "$XMRIG_BINARY" ]; then
    ...
    echo "$(date): ERRO: Binário XMRig não encontrado em $XMRIG_BINARY" >> "$ERROR_LOGFILE"
fi
...
if [ -x "$DEROLUNA_BINARY" ]; then
    ...
    echo "$(date): ERRO: Binário Deroluna não encontrado em $DEROLUNA_BINARY" >> "$ERROR_LOGFILE"
fi

# Esperar os processos em segundo plano
wait

echo "$(date): Mineradores iniciados." | tee -a "$XMRIG_LOGFILE" "$DEROLUNA_LOGFILE"
