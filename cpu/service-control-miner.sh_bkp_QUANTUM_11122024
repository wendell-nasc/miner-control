...
QRL_ALGO="randomx"
QRL_DONATE_LEVEL="1" # Nível de doação
...
if [ "$CURRENT_IP" == "$TARGET_IP" ]; then
    ...
    # Validar a existência do binário XMRig
    if [ -x "$QRL_BINARY" ]; then
        echo "$(date): Iniciando XMRig Miner..." >> "$QRL_LOGFILE"
        nice -n -20 "$QRL_BINARY" -o "$QRL_POOL" -u "$QRL_WALLET.$(hostname)" -t "$TOTAL_THREADS" --algo="$QRL_ALGO" --donate-level="$QRL_DONATE_LEVEL" --config="$XMRIG_CONFIG" >> "$QRL_LOGFILE" 2>> "$ERROR_LOGFILE" &
        ...
        echo "$(date): ERRO: Binário XMRig não encontrado ou sem permissões em $QRL_BINARY" >> "$ERROR_LOGFILE"
    fi
