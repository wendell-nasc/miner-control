...
echo "Comando: $SRB_PATH --disable-gpu --algorithm $ALGO_SRB --pool $POOL_SRB --wallet $WALLET_SRB.$WORKER_NAME --keepalive true" >> "$SRB_LOGFILE"

"$SRB_PATH" --disable-gpu --algorithm "$ALGO_SRB"   \
...
echo "Comando: $CPUMINER_PATH -a $ALGO_CPUMINER -o $POOL_CPUMINER -u $WALLET_CPUMINER.$WORKER_NAME " >> "$CPUMINER_LOGFILE"

"$CPUMINER_PATH" -a "$ALGO_CPUMINER" -o "$POOL_CPUMINER" -u "$WALLET_CPUMINER.$WORKER_NAME"  \
